...
import React, { InputHTMLAttributes, LegacyRef, ReactNode, forwardRef, useRef, useState } from 'react';
...
const TextField = forwardRef(
    (
        ...
        const disabledClass = disabled ? " cursor-not-allowed !bg-lightGray !text-primaryGray" : "";
        ...
        const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            // } 
                ...
                newValue = newValue.replace(/(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g, "");
            }
        ...
        return (
            <div className={`flex flex-col gap-1 ${className}`}>
                ...
                <div className="relative">
                    ...
                    />
                    {rightIcon && (
                        <div className="absolute inset-y-0 right-[21px] flex items-center cursor-pointer"
                            onClick={onClick}>
                            {rightIcon}
                        </div>
                    )}
                    {LeftIcon && (
                        <div className="absolute inset-y-0 left-[16px] flex items-center cursor-pointer"
                            onClick={onClick}>
                            {LeftIcon}
                        </div>
                    )}
