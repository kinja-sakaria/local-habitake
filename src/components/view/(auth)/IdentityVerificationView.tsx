...
export default function IdentityVerificationView() {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            if (!allowedTypes.includes(selectedFile.type)) {
                alert('Only PNG, JPG, JPEG, or WEBP images are allowed. SVG is not supported.');
                return;
            }
    ...
    return (
        <>
            ...
            {/* Form for Indentity Verification */}
            <div className='pt-10'>
                <form>
                    <div className='pt-6'>
                        <IdDropdown
                            label="ID Proof"
                            options={["Addhar", "Passport", "Driving License"]}
                            defaultValue="Select ID Proof"
                            onChange={(val) => console.log("Selected:", val)} />
                    </div>

                    <div className="pt-6">
                        <div className="relative border-2 border-dashed border-disableGray rounded-md w-full h-[220px] flex flex-col items-center justify-center bg-lightGray overflow-hidden">
                            <input
                                type="file"
